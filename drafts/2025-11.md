...
Burn is both a tensor library and a deep learning framework optimized for numerical computing, model inference and model training. The 0.19.0 and 0.19.1 [releases](https://github.com/tracel-ai/burn/releases) bring major improvements to enable efficient distributed training, quantization, and CPU support.

### [Polars 0.52.0](https://crates.io/crates/polars) 
Polars is an analytical query engine written for DataFrames. Highlights of [this release](https://github.com/pola-rs/polars/releases/tag/rs-0.52.0) include the addition of `LazyFrame.{sink,collect}_batches` and deterministic import order for Python Polars package variants.

### [Zarrs 0.22.7](https://crates.io/crates/zarrs)
Zarrs is a Rust library for the Zarr storage format for multidimensional arrays and metadata. Mutliple [releases](https://github.com/zarrs/zarrs/releases) add functionality to `Array`, `node`, `Group`, and `DataType`, as well as `hierarchy` module.
